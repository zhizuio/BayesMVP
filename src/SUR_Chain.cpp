...
SUR_Chain::SUR_Chain( std::shared_ptr<arma::mat> data_, std::shared_ptr<arma::mat> mrfG_, unsigned int nObservations_,
                     ...
                     w_Sampler_Type w_sampler_type_ , Beta_Type beta_type_ , Covariance_Type covariance_type_ , bool output_CPO , int maxThreads , int tick,
...
covariance_type(covariance_type_), gamma_type(gamma_type_),beta_type(beta_type_),gamma_sampler_type(gamma_sampler_type_),w_sampler_type(w_sampler_type_)
{
...
surData.outcomesIdx,surData.VSPredictorsIdx,surData.fixedPredictorsIdx,surData.missingDataArrayIdx,surData.completeCases,
          gamma_sampler_type_,gamma_type_,w_sampler_type_,beta_type_,covariance_type_,output_CPO,maxThreads,tick,externalTemperature){ }

SUR_Chain::SUR_Chain( Utils::SUR_Data& surData, double externalTemperature ):
SUR_Chain(surData.data,surData.mrfG,surData.nObservations,surData.nOutcomes,surData.nVSPredictors,surData.nFixedPredictors,
surData.outcomesIdx,surData.VSPredictorsIdx,surData.fixedPredictorsIdx,surData.missingDataArrayIdx,surData.completeCases,
          Gamma_Sampler_Type::bandit , Gamma_Type::hotspot , w_Sampler_Type::MH , Beta_Type::independent , Covariance_Type::HIW , false,
          externalTemperature){ }
...
w_Sampler_Type SUR_Chain::getwSamplerType(){ return w_sampler_type ; }
...
void SUR_Chain::sigmaRhoInit()
{
    ...
    arma::mat init = arma::ones<arma::mat>(nOutcomes,nOutcomes);
...
void SUR_Chain::zInit()
{
    arma::mat init = arma::zeros<arma::mat>(nObservations,nOutcomes);
    zInit( init );
}

void SUR_Chain::zInit( arma::mat& z_init )
{
    zInit( z_init, sigmaRho );
}

void SUR_Chain::zInit( arma::mat& z_init, arma::mat& sigmaRho_init )
{
    Z = z_init;
    arma::vec rhoU_init = arma::zeros<arma::vec>( nObservations );
    
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        Z.col(k) = Distributions::randMvNormal( rhoU_init , sigmaRho_init(k,k) );
    }
}
    ...
    for( unsigned q=0; q < externalJT.perfectCliqueSequence.size(); ++q )
    {
        ...
        for( unsigned int t=0; t<Res_q.size(); ++t )
        {
            ...
            if( nConditioninIndexes > 0 )
            {
                ...
                //rhoVar = (1./tau_) * arma::diagmat( arma::ones(nConditioninIndexes) );
            ...
            case Beta_Type::independent :
            {
                for(unsigned int k=0; k<nOutcomes ; ++k)
                {
                    ...
                    logP += Distributions::logPDFNormal( externalBeta(VS_IN_k,singleIdx_k) ,
                                                        ...
                                                        //w_ * arma::diagmat( arma::ones(VS_IN_k.n_elem) ) );
                }
                break;
            }
...
arma::mat SUR_Chain::predLikelihood( )
{
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
        ...
        {
            predLik(i,k) = std::exp( Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , (XB(i,k)+rhoU(i,k)) , sigmaRho(k,k)) );
        }
...
double SUR_Chain::logLikelihood( )
{
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        ...
        {
            // do we update Z here?
            //double tmpZ = Distributions::randNormal( XB(i,k) + rhoU(i,k) , sigmaRho(k,k) );
            double tmpZ = Distributions::CDFNormal( Z(i,k) );
            if( (*data)(i, (*outcomesIdx)(k)) + tmpZ > 1.)
            {
                logP += std::log(tmpZ);
            } else {
                if( (*data)(i, (*outcomesIdx)(k)) + tmpZ < 1. )
                ...
                //logP += std::log(1.0e-5);
            //logP += Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , XB(i,k) + rhoU(i,k) , sigmaRho(k,k) );
            /*if( Z(i,k) >= 0. )
            {
                logP += (*data)(i, (*outcomesIdx)(k)) * Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , XB(i,k) + rhoU(i,k) , sigmaRho(k,k) );
                //logP += (*data)(i, (*outcomesIdx)(k)) * Distributions::logPDFNormal( XB(i,k) + rhoU(i,k), 0, 1 );
            } else {
                logP += ( 1. - (*data)(i, (*outcomesIdx)(k)) ) * Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , XB(i,k) + rhoU(i,k) , sigmaRho(k,k) );
                //logP += ( 1. - (*data)(i, (*outcomesIdx)(k)) ) * Distributions::logPDFNormal( XB(i,k) + rhoU(i,k), 0, 1 );
            }*/
        }
    }
...
// compute log-likelihood of the latent responses, also jointly Y,Z conditional on others
double SUR_Chain::logLikelihood( const arma::mat& externalXB , const arma::mat& externalRhoU , const arma::mat&  externalSigmaRho )
{
    double logP = 0.;
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        for( unsigned int i=0; i<nObservations; ++i)
        {
            double tmpZ = Distributions::randNormal( externalXB(i,k) + externalRhoU(i,k) , externalSigmaRho(k,k) );
            //double tmpZ = externalXB(i,k) + externalRhoU(i,k);
            //if( ((*data)(i, (*outcomesIdx)(k)) + tmpZ > 0.) && ((*data)(i, (*outcomesIdx)(k)) + tmpZ < 1.) ) // rejection sampler
            if( !( ((*data)(i, (*outcomesIdx)(k)) == 1. && tmpZ > 0.) || ((*data)(i, (*outcomesIdx)(k)) == 0. && tmpZ <= 0.) ) ) // rejection sampler
            {
                tmpZ = Z(i,k);
            }
            tmpZ = Distributions::CDFNormal( tmpZ );
            if( (*data)(i, (*outcomesIdx)(k)) + tmpZ > 1.)
            {
                logP += std::log(tmpZ);
            } else {
                ...
                // logP += std::log(1.0e-5);
            //logP += Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , externalXB(i,k) + externalRhoU(i,k) , externalSigmaRho(k,k) );
            /*if( Z(i,k) >= 0. )
            {
                logP += (*data)(i, (*outcomesIdx)(k)) * Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , externalXB(i,k) + externalRhoU(i,k) , externalSigmaRho(k,k) );
                //logP += (*data)(i, (*outcomesIdx)(k)) * Distributions::logPDFNormal( externalXB(i,k) + externalRhoU(i,k), 0, 1 );
            } else
                logP += ( 1. - (*data)(i, (*outcomesIdx)(k)) ) * Distributions::logPDFNormal( (*data)(i, (*outcomesIdx)(k)) , externalXB(i,k) + externalRhoU(i,k) , externalSigmaRho(k,k) );
                //logP += ( 1. - (*data)(i, (*outcomesIdx)(k)) ) * Distributions::logPDFNormal( externalXB(i,k) + externalRhoU(i,k), 0, 1 );
            */
        }
    }
                                          ...
                                          const arma::umat&  externalGammaMask , const arma::mat& externalXB , const arma::mat& externalU , arma::mat& mutantRhoU )
{
        ...
        case Covariance_Type::HIW :
        {
            ...
            for( unsigned q=0; q < externalJT.perfectCliqueSequence.size(); ++q )
            {
                ...
                for( unsigned int t=0; t<Res_q.size(); ++t )
                {
                    ...
                    if( mutantSigmaRho(l,l) > 1.0 ) mutantSigmaRho(l,l) = 1.0;
                    ...
                    {
                        mutantSigmaRho(l,l) = Distributions::randIGamma( a , b );
                        // restrict the parameter due to non-identifiability issue of MVP
                        if( mutantSigmaRho(l,l) > 1.0 ) mutantSigmaRho(l,l) = 1.0;
                        
                        logP += Distributions::logPDFIGamma( mutantSigmaRho(l,l), a , b );
                    } else
                        mutantSigmaRho(l,l) = 1.0;
                    */
                    // No need to infer mutantSigmaRho(k,k), since we assume the latent responses follow standard normal distribution
                    mutantSigmaRho(l,l) = 1.0;
                    ...
                    if( nConditioninIndexes > 0 )
                    {
                        ...
                        arma::vec tmpRho = Distributions::randMvNormal( rhoMean.t() , mutantSigmaRho(l,l) * rhoVar );
                        // restrict the parameter due to non-identifiability issue of MVP
                        tmpRho.elem(arma::find(tmpRho >= 1.0)).fill(0.99);
                        tmpRho.elem(arma::find(tmpRho <= -1.0)).fill(-0.99);
                        /*tmpRho.elem(arma::find(tmpRho >=  mutantSigmaRho(k,k))).fill( mutantSigmaRho(k,k) );
                        tmpRho.elem(arma::find(tmpRho <= -mutantSigmaRho(k,k))).fill( -mutantSigmaRho(k,k) );*/
                        mutantSigmaRho( conditioninIndexes , singleIdx_l ) = tmpRho;
        ...
        case Covariance_Type::IW :
        {
            ...
            for( unsigned int k=0; k<nOutcomes; ++k )
            {
                ...
                if( b > 0. )
                {
                    mutantSigmaRho(k,k) = Distributions::randIGamma( a , b );
                    // restrict the parameter due to non-identifiability issue of MVP
                    if( mutantSigmaRho(k,k) > 1.0 ) mutantSigmaRho(k,k) = 1.0;
                    
                    logP += Distributions::logPDFIGamma( mutantSigmaRho(k,k), a , b );
                } else
                    mutantSigmaRho(k,k) = 1.0;
                */
                // No need to infer mutantSigmaRho(k,k), since we assume the latent responses follow standard normal distribution
                mutantSigmaRho(k,k) = 1.0;
                ...
                if( nConditioninIndexes > 0 )
                {
                    ...
                    arma::vec tmpRho = Distributions::randMvNormal( rhoMean.t() , mutantSigmaRho(k,k) * rhoVar );
                    // restrict the parameter due to non-identifiability issue of MVP
                    tmpRho.elem(arma::find(tmpRho >= 1.0)).fill(0.99);
                    tmpRho.elem(arma::find(tmpRho <= -1.0)).fill(-0.99);
                    mutantSigmaRho( conditioninIndexes , singleIdx_k ) = tmpRho;
    ...
    //mutantSigmaRho.elem(arma::find(mutantSigmaRho > 1.0e+3)).fill(1.0e+3); // Should we threshold this?
    //Testing*/mutantSigmaRho = arma::eye<arma::mat>(nOutcomes, nOutcomes);
                                          ...
                                          const arma::umat&  externalGammaMask , arma::mat& mutantXB , arma::mat& mutantU , arma::mat& mutantRhoU )
{
    ...
    mutantBeta.fill(0.); // to be sure
    //externalSigmaRho.diag() = arma::ones( nOutcomes );  // to be sure
    ...
    if(externalGammaMask.n_rows>0)
    {
        ...
        arma::vec xtxMultiplier(nOutcomes);
        //arma::mat z_tilde = data->cols( (*outcomesIdx) ) - mutantRhoU ;
        arma::mat z_tilde = Z - mutantRhoU ;
        
        // prepare posterior full conditional's hyperparameters
        z_tilde.each_row() /= ( externalSigmaRho.diag().t()) ; // divide each col by the corresponding element of sigma
        xtxMultiplier(xi(nOutcomes-1)) = 0;
        // z_tilde.col(xi(nOutcomes-1)) is already ok;
        ...
        for( unsigned int k=0; k < (nOutcomes-1); ++k)
        {
            ...
            for(unsigned int l=k+1 ; l<nOutcomes ; ++l)
            {
                ...
                z_tilde.col(xi(k)) -= (  externalSigmaRho(xi(l),xi(k)) /  externalSigmaRho(xi(l),xi(l)) ) *
                ( mutantU.col(xi(l)) - mutantRhoU.col(xi(l)) +  externalSigmaRho(xi(l),xi(k)) *
                 ( mutantU.col(xi(k)) - Z.col( xi(k) ) ) );
            }
            ...
            if(VS_IN_k.n_elem>0)
            {
                ...
                switch ( beta_type )
                {
                    case Beta_Type::gprior :
                    {
                        ...
                        ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * z_tilde.col(k) / temperature );
                    ...
                    {
                        if( preComputedXtX )
                            arma::inv_sympd( W_k ,  ( XtX(VS_IN_k,VS_IN_k) / temperature ) * ( 1./ externalSigmaRho(k,k) + xtxMultiplier(k) ) +
                                            (1./w)*arma::eye<arma::mat>(VS_IN_k.n_elem,VS_IN_k.n_elem) );
                                            //(1./w)*arma::diagmat( arma::ones(VS_IN_k.n_elem) ) );
                        else
                            arma::inv_sympd( W_k ,  ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * data->cols( (*predictorsIdx)(VS_IN_k) ) ) * ( 1./ externalSigmaRho(k,k) + xtxMultiplier(k) ) + 
                                            (1./w)*arma::eye<arma::mat>(VS_IN_k.n_elem,VS_IN_k.n_elem) );
                                            //(1./w)*arma::diagmat( arma::ones(VS_IN_k.n_elem) ) );
                        
                        mu_k = W_k * ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * z_tilde.col(k) / temperature ) ;
   ...
   // externalSigmaRho.diag() = arma::ones( nOutcomes );  // to be sure
    ...
    if(externalGammaMask.n_rows>0)
    {
        ...
        if(VS_IN_k.n_elem>0)
        {
            ...
            //arma::vec z_tilde = data->col( (*outcomesIdx)(k) ) - mutantRhoU.col(k) ;
            arma::vec z_tilde = Z.col(k) - mutantRhoU.col(k) ;
            
            // prepare posterior full conditional's hyperparameters
            xtxMultiplier = 0;
            z_tilde /=  externalSigmaRho(k,k);
            ...
            for(unsigned int l=k_idx+1 ; l<nOutcomes ; ++l)
            {
                ...
                z_tilde -= (  externalSigmaRho(xi(l),k) /  externalSigmaRho(xi(l),xi(l)) ) *
                ( mutantU.col(xi(l)) - mutantRhoU.col(xi(l)) +  externalSigmaRho(xi(l),k) * 
                 //( mutantU.col(k) - data->col( (*outcomesIdx)(k) ) ) );
                 ( mutantU.col(k) - Z.col(k) ) );
            }
            ...
            if( preComputedXtX )
            {
                switch ( beta_type )
                {
                    ...
                    case Beta_Type::independent :
                    {
                        arma::inv_sympd( W_k ,  ( XtX(VS_IN_k,VS_IN_k) / temperature ) * ( 1./ externalSigmaRho(k,k) + xtxMultiplier ) + 
                                        (1./w)*arma::eye<arma::mat>(VS_IN_k.n_elem,VS_IN_k.n_elem) );
                                        //(1./w)*arma::diagmat( arma::ones(VS_IN_k.n_elem) ) );
                        break;
                    }
                ...
                switch ( beta_type )
                {
                    ...
                    case Beta_Type::independent :
                    {
                        arma::inv_sympd( W_k ,  ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * data->cols( (*predictorsIdx)(VS_IN_k) ) ) * ( 1./ externalSigmaRho(k,k) + xtxMultiplier ) + 
                                        (1./w)*arma::eye<arma::mat>(VS_IN_k.n_elem,VS_IN_k.n_elem) );
                                        //(1./w)*arma::diagmat( arma::ones(VS_IN_k.n_elem) ) );
                        break;
                    }
            ...
            mu_k = W_k * ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * z_tilde / temperature ) ;
    ...
    if(externalGammaMask.n_rows>0)
    {
        ...
        for( unsigned int k=0; k < (nOutcomes-1); ++k)
        {
            ...
            for(unsigned int l=k+1 ; l<nOutcomes ; ++l)
            {
                xtxMultiplier(xi(k)) += pow( externalSigmaRho(xi(l),xi(k)),2) /  externalSigmaRho(xi(l),xi(l));
                z_tilde.col(xi(k)) -= (  externalSigmaRho(xi(l),xi(k)) /  externalSigmaRho(xi(l),xi(l)) ) *
                ( U.col(xi(l)) - mutantRhoU.col(xi(l)) +  externalSigmaRho(xi(l),xi(k)) * 
                 ( mutantU.col(xi(k)) - Z.col( xi(k) ) ) );
            }
...
// sample Z given Beta, rhoU and sigmaRho
void SUR_Chain::sampleZ( const arma::mat& externalXB , const arma::mat& externalRhoU , const arma::mat& externalSigmaRho )
{
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        //Z.col(k) = Distributions::randMvNormal( (externalXB.col(k) + externalRhoU.col(k)) , externalSigmaRho(k,k));
        // normal distribution truncated to the positive orthant (Chib & Greenberg, 1998)
        for( unsigned int i=0; i<nObservations; ++i)
        {
            double tmpZ = Distributions::randNormal( externalXB(i,k) + externalRhoU(i,k), externalSigmaRho(k,k) );
            if( ((*data)(i, (*outcomesIdx)(k)) == 1. && tmpZ > 0.) || ((*data)(i, (*outcomesIdx)(k)) == 0. && tmpZ <= 0.) ) // rejection sampler
...
void SUR_Chain::sampleZ()
{
    sampleZ( XB , rhoU , sigmaRho );
}
...
void SUR_Chain::stepJT()
{
    ...
    for( unsigned int iter=0; iter < n_updates_jt; ++iter )
    {
        ...
        if( updateBeta )
        {
            ...
            proposedLikelihood = logLikelihood( proposedXB , proposedRhoU , proposedSigmaRho );
            ...
            proposedLikelihood = logLikelihood( XB , proposedRhoU , proposedSigmaRho );
...
{
    double a = a_w + 0.5*( gammaMask.n_rows ); // divide by temperature if the prior on gamma is tempered
...
void SUR_Chain::stepW0Gibbs()
{
    ...
    double b = b_w + 0.5*( arma::accu( arma::square(beta.rows(nFixedPredictors,nFixedPredictors+nVSPredictors-1)) ) );

    // std::cout << a_w << " -> " << a << "   ---   "<< b_w << " -> " << b << std::endl;
    // std::cout << arma::nonzeros(beta).t() << std::endl; std::cin >> w;

    w = Distributions::randIGamma( a , b );
    //w = 1. / Distributions::randIGamma( a , b );
    logPW(); // update its prior value
    ...
    if( nFixedPredictors > 0 )
    {
        ...
        //w0 = 1. / Distributions::randGamma( a0 , b0 );
...
void SUR_Chain::stepGamma()
{
    ...
    double proposedLikelihood = logLikelihood( proposedXB , proposedRhoU , sigmaRho );
    ...
    /*std::cout << "Debug stepGamma: logAccProb=" << logAccProb << "; proposedGamma=" << proposedGamma(0,0) <<
    "\n  logProposalRatio=" << logProposalRatio <<
    "; proposedGammaPrior=" << proposedGammaPrior <<
    "; logP_gamma=" << logP_gamma <<
...
int SUR_Chain::exchangeGamma_step( std::shared_ptr<SUR_Chain>& that )
{
    ...
    double logLik_1 = this->logLikelihood( that->getXB() , rhoU_1 , this->getSigmaRho() );   // note that this and that lik are
    double logLik_2 = that->logLikelihood( this->getXB() , rhoU_2 , that->getSigmaRho() ); // important because of temperature
...
int SUR_Chain::exchangeJT_step( std::shared_ptr<SUR_Chain>& that )
{
    ...
    double logLik_1 = this->logLikelihood( this->getXB() , rhoU_1 , that->getSigmaRho() ); // note that this and that lik are
    double logLik_2 = that->logLikelihood( that->getXB() , rhoU_2 , this->getSigmaRho() );   // important because of temperature
...
int SUR_Chain::adapt_crossOver_step( std::shared_ptr<SUR_Chain>& that )
{
    ...
    double logLikFirst = this->logLikelihood( XB_XO[0] , rhoU_XO[0] , this->getSigmaRho() );
    double logLikSecond = that->logLikelihood( XB_XO[1] , rhoU_XO[1] , that->getSigmaRho() );
...
arma::mat SUR_Chain::createXB( const arma::umat&  externalGammaMask , const arma::mat&  externalBeta )
{
    ...
    // Testing
    //externalXB.elem(arma::find(externalXB > 50.)).fill(50.);
    //externalXB.elem(arma::find(externalXB < -50.)).fill(-50.);
...
arma::mat SUR_Chain::createU( const arma::mat& externalXB )
{
    ...
    arma::mat externalU = Z - externalXB;
    return externalU;
}
...
void SUR_Chain::updateQuantities()
{
    ...
    updateU();
    updateRhoU();
    //updateZ(); // Should this be put a few steps before?
}
