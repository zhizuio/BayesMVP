...
HRR_Chain(surData.data,surData.mrfG,surData.nObservations,surData.nOutcomes,surData.nVSPredictors,surData.nFixedPredictors,
surData.outcomesIdx,surData.VSPredictorsIdx,surData.fixedPredictorsIdx,surData.missingDataArrayIdx,surData.completeCases,
          Gamma_Sampler_Type::bandit , Gamma_Type::hotspot , w_Sampler_Type::MH , Beta_Type::independent , Covariance_Type::IG , false ,
          externalTemperature){ }
...
w_Sampler_Type HRR_Chain::getwSamplerType(){ return w_sampler_type ; }
...
arma::mat& HRR_Chain::getBeta() //const
{
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        ...
        if(VS_IN.n_elem>0)
        {
            ...
            arma::vec mu_k = W_k * ( data->cols( VS_IN ).t() * Z.col(k) / temperature ); // we divide by temp later
...
void HRR_Chain::setBeta( arma::mat&  externalBeta )
{
    beta =  externalBeta ;
}

void HRR_Chain::setBeta( arma::mat&  externalBeta , double logP_beta_ )
{
    beta =  externalBeta ;
...
void HRR_Chain::piInit()
{
    ...
    switch ( gamma_type )
    {
        ...
        case Gamma_Type::hierarchical :
            /*for(unsigned i=0; i<nVSPredictors; i++)
            {
                init(i) = Distributions::randBeta(.01, (double)nOutcomes-1.);
            }*/
            init = init * 0.02;
...
void HRR_Chain::betaInit()
{
    arma::mat init = arma::zeros<arma::mat>(nFixedPredictors+nVSPredictors, nOutcomes);
    beta = init;
}

void HRR_Chain::zInit()
{
    //arma::mat init = arma::zeros<arma::mat>(nObservations, nOutcomes);
    //zInit( init );
    arma::mat XB_init = data->cols( *predictorsIdx ) * beta;
    Z = arma::zeros<arma::mat>(nObservations, nOutcomes);
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        Z.col(k) = Distributions::randMvNormal( XB_init.col(k) , 1. );
    }
}
/*
void HRR_Chain::zInit( arma::mat& z_init )
{
    zInit( z_init, 1. );
}

void HRR_Chain::zInit( arma::mat& z_init, double Sigma_init )
{
    Z = z_init;
    arma::vec XB_init = data->cols( *predictorsIdx ) * beta;
    
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        Z.col(k) = Distributions::randMvNormal( XB_init , Sigma_init );
    }
}*/
...
double HRR_Chain::logLikelihood( )
{
    ...
    // zMean is needed if y is not standardized. This might cause MCMC stopped
    //arma::mat zMean = data->cols( *outcomesIdx );
    arma::mat zMean = Z;
    for( unsigned k=0; k<nOutcomes; k++)
    {
        zMean.col(k) = arma::mean(zMean.col(k)) * arma::ones(nObservations);
    }
    //Testing*/ zMean.fill(0.);
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        ...
        arma::vec mu_k = W_k * ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * ( Z.col(k) - zMean.col(k)) ) / temperature; // we divide by temp later
        
        double a_sigma_k = a_sigma + 0.5*(double)nObservations / temperature;
        double b_sigma_k = b_sigma + 0.5* arma::as_scalar( (( Z.col(k) - zMean.col(k) ).t() * (Z.col(k) - zMean.col(k) )) -
                                                          ( mu_k.t() * data->cols( (*predictorsIdx)(VS_IN_k) ).t() * ( Z.col(k) - zMean.col(k) ) ) ) / temperature; // S(gamma) in Equation (9) of Bottolo & Richardson (2010)
        
        // sample sigma_k
        //if( b_sigma_k < 0. ) std::cout << "loglikelihood() b_sigma_k < 0! max(Z)=" << max(Z) << "; max(zMean)=" << max(zMean) << std::endl;
        /*double sigma_k = 1.;
        if( b_sigma_k > 0. )
        {
            sigma_k = Distributions::randIGamma( a_sigma_k , b_sigma_k );
            //if( sigma_k > 1. )
            //    sigma_k = 1.; // constraint due to non-identifiability of MVP
        } else
            sigma_k = 1.;*/
        ...
        // sample beta
        //arma::vec mu_k = W_k * data->cols( VS_IN_k ).t() * Z.col(k);
        arma::uvec singleIdx_k = { k };
        beta.submat(VS_IN_k, singleIdx_k) = Distributions::randMvNormal( mu_k , W_k );
        //beta.elem(arma::find(beta > 10.0)).fill(10.0);
        //beta.elem(arma::find(beta < -10.0)).fill(-10.0);
        
        // sample Z and compute log-likelihood
        arma::vec tmpZ = Distributions::randMvNormal( data->cols( (*predictorsIdx)(VS_IN_k) ) * beta.submat(VS_IN_k, singleIdx_k) , sigma_k);
        for( unsigned int i=0; i<nObservations; ++i)
        {
            if( ((*data)(i, (*outcomesIdx)(k)) == 1. && tmpZ(i) > 0.) || ((*data)(i, (*outcomesIdx)(k)) == 0. && tmpZ(i) <= 0.) ) // rejection sampler
            {
                Z(i,k) = tmpZ(i);
            }
            double cdfZ = Distributions::CDFNormal( Z(i,k) );
            if( (*data)(i, (*outcomesIdx)(k)) + cdfZ > 1.)
            {
                logP += std::log(cdfZ);
            } else {
                if( (*data)(i, (*outcomesIdx)(k)) + cdfZ < 1. )
        ...
        if( output_CPO && (temperature = 1.) ){
            for( unsigned int i=0; i<nObservations; ++i )
            {
                ...
                mu_scale = (data->col(k))(i) - arma::as_scalar(arma::conv_to<arma::vec>::from(data->row(i))((*predictorsIdx)(VS_IN_k)).t() *mu_k);
                W_scale = b_sigma_k/a_sigma_k * ( 1. + arma::as_scalar(arma::conv_to<arma::vec>::from(data->row(i))((*predictorsIdx)(VS_IN_k)).t() * W_k * arma::conv_to<arma::vec>::from(data->row(i))((*predictorsIdx)(VS_IN_k))) );
...
double HRR_Chain::logLikelihood( const arma::umat&  externalGammaMask )
{
    ...
    // zMean is needed if y is not standardized
    //arma::mat zMean = data->cols( *outcomesIdx );
    arma::mat zMean = Z;
    for( unsigned k=0; k<nOutcomes; k++)
    {
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        ...
        W_k = arma::inv_sympd( W_k );
        arma::vec mu_k = W_k * ( data->cols( (*predictorsIdx)(VS_IN_k) ).t() * ( Z.col(k) - zMean.col(k)) ) / temperature;
        
        //double a_sigma_k = a_sigma + 0.5*(double)nObservations / temperature;
        //double b_sigma_k = b_sigma + 0.5* arma::as_scalar( (( Z.col(k) - zMean.col(k) ).t() * (Z.col(k) - zMean.col(k) )) - ( mu_k.t() * data->cols( (*predictorsIdx)(VS_IN_k) ).t() * ( Z.col(k) - zMean.col(k) ) ) ) / temperature;
        ...
        //arma::vec mu_k = W_k * data->cols( VS_IN_k ).t() * Z.col(k);
        arma::uvec singleIdx_k = { k };
        arma::vec mutantBeta = Distributions::randMvNormal( mu_k , W_k );
        //mutantBeta.elem(arma::find(mutantBeta > 10.0)).fill(10.0);
        //mutantBeta.elem(arma::find(mutantBeta < -10.0)).fill(-10.0);
        
        // sample Z and compute log-likelihood
        arma::vec tmpZ = Distributions::randMvNormal( data->cols( (*predictorsIdx)(VS_IN_k) ) * mutantBeta , sigma_k);
        for( unsigned int i=0; i<nObservations; ++i)
        {
            double tmpZZ;
            if( ((*data)(i, (*outcomesIdx)(k)) == 1. && tmpZ(i) > 0.) || ((*data)(i, (*outcomesIdx)(k)) == 0. && tmpZ(i) <= 0.) ) // rejection sampler
            ...
            double cdfZ = Distributions::CDFNormal( tmpZZ );
            if( (*data)(i, (*outcomesIdx)(k)) + cdfZ > 1.)
            {
                logP += std::log(cdfZ);
            } else {
...
double HRR_Chain::logLikelihood( arma::umat& externalGammaMask , const arma::umat& externalGamma ) // gammaMask , gamma
{
    ...
    for( unsigned int k=0; k<nOutcomes; ++k)
    {
        ...
        {
            double tmpZZ;
            if( ((*data)(i, (*outcomesIdx)(k)) + tmpZ(i) > 1.) || ((*data)(i, (*outcomesIdx)(k)) + tmpZ(i) < 0.) ) // rejection sampler
            {
                tmpZZ = tmpZ(i);
            } else {
                tmpZZ = Z(i,k);
...
// Gibbs sampling for slab's variance w, also for random effects' w0 if needed
void HRR_Chain::stepWGibbs()
{
    double a = a_w + 0.5*( gammaMask.n_rows - nFixedPredictors*nOutcomes );
    ...
    {
        double a0 = a_w0 + 0.5*( nFixedPredictors*nOutcomes );
        double b0 = b_w0 + 0.5*( arma::accu( arma::square(beta.rows(0,nFixedPredictors-1)) ) );
...
void HRR_Chain::stepWMH()
{
    //std::cout << "stepWMH() - start w=" << w << "; w0=" << w0 << std::endl;
    // Metropolis-Hastings samplers for w and w0
    double proposedW = std::exp( std::log(w) + Distributions::randNormal(0.0, var_w_proposal) );
    
    double proposedWPrior = logPW( proposedW );
    double proposedLikelihood = logLikelihood( gammaMask , proposedW , w0 , a_sigma , b_sigma );
    
    double logAccProb = (proposedWPrior + proposedLikelihood) - (logP_w + log_likelihood);
    
    /*std::cout << "  proposedW=" << proposedW <<
    "; logAccProb=" << logAccProb <<
    "; proposedWPrior=" << proposedWPrior <<
    ...
    if( nFixedPredictors > 0 )
    {
        ...
        logAccProb = (proposedW0Prior + proposedLikelihood) - (logP_w0 + log_likelihood);
    ...
    //std::cout << "stepWMH() - end w=" << w << "; w0=" << w0 << "; log_likelihood=" << log_likelihood << std::endl;
}
/*
void HRR_Chain::stepW0()
{
  //double proposedW = std::exp( std::log(w) + Distributions::randNormal(0.0, var_w_proposal) ); // no need to sample a new w
  double proposedW0 = std::exp( std::log(w0) + Distributions::randNormal(0.0, var_w0_proposal) );
...
void HRR_Chain::step()
{
    ...
    //stepWMH();
    //stepWGibbs();
    switch ( w_sampler_type )
    {
        case w_Sampler_Type::MH :
            stepWMH();
            break;
            
        case w_Sampler_Type::gibbs :
            stepWGibbs();
            break;
    }
...
void HRR_Chain::swapBeta( std::shared_ptr<HRR_Chain>& that )
{
