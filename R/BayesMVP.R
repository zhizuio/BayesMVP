...
#' for details
#' @param wSampler string indicating the type of sampler for the variance w 
#' (and w0) of regression coefficients. It is either \code{MH} (default) or 
#' \code{gibbs}
...
#' a_eta, b_eta, a_o, b_o, a_pi, b_pi, a_w, b_w and c1. Their default values are
#' a_w=2, b_w=5, c1=1, a_omega=2, b_omega=1, a_o=2, b_o=p-2, a_pi=2, b_pi=1, nu=s+2,
...
BayesMVP <- function(data = NULL, Y, X, X_0 = NULL,
  ...
  ret$status <- BayesMVP_internal(
    ...
    gammaSampler, gammaInit, wSampler, betaPrior,
