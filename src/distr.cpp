...
namespace Distributions{
    ...
    arma::vec randMvNormal(const arma::vec &m, const double &sigma)
    {
        unsigned int d = m.n_elem;
        arma::mat Sigma = arma::diagmat( arma::vec(d, arma::fill::value(sigma)) );
        
        /*
        if(!Sigma.is_symmetric())
            std::cout << "...sigma symetric: " << Sigma.is_symmetric() << std::endl;
         */
