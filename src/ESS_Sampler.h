...
public:
    ...
    ESS_Sampler( Utils::SUR_Data& surData , unsigned int nChains_ , double temperatureRatio ,
                Gamma_Sampler_Type gamma_sampler_type, Gamma_Type gamma_type,
                w_Sampler_Type w_sampler_type, Beta_Type beta_type, Covariance_Type covariance_type,
                bool output_CPO , int maxThreads , int tick , unsigned int burnin_ );
    
    ESS_Sampler( Utils::SUR_Data& surData , unsigned int nChains_ , double temperatureRatio ) :
    ESS_Sampler( surData , nChains_ , 1.2 ,
    Gamma_Sampler_Type::bandit, Gamma_Type::hotspot , w_Sampler_Type::MH, Beta_Type::independent , Covariance_Type::HIW , false , 1 , 1000 , 0 ){}
...
global_count(0)
{
    ...
    for( unsigned int i=0; i<nChains; ++i )
        chain[i] = std::make_shared<T>( surData ,
                                       gamma_sampler_type, gamma_type, w_sampler_type, beta_type, covariance_type, output_CPO, maxThreads, tick,
